...
class HighProbStrategy:
    ...
    def update_positions(self):
        """Update held positions from API, preserving entry_price and entry_time."""
        positions_response = self.trader.get_positions()
        market_positions = getattr(positions_response, "market_positions", [])

        for pos in market_positions:
            ticker = pos.ticker
            existing = self.held_positions.get(ticker, {})
            self.held_positions[ticker] = {
                "side": "yes" if pos.position > 0 else "no",
                "contracts": abs(pos.position),
                "entry_price": existing.get("entry_price"),  # preserve if known
                "entry_time": existing.get("entry_time"),    # preserve if known
            }

        # Remove positions no longer held at Kalshi
        active_tickers = {pos.ticker for pos in market_positions}
        for ticker in list(self.held_positions.keys()):
            if ticker not in active_tickers:
                self.held_positions.pop(ticker)
    ...
    def _passes_basic_filters(self, market) -> bool:
        ...
        illiquid = (yes_bid == 0 and yes_ask == 100) or (no_bid == 0 and no_ask == 100)
        has_volume = (getattr(market, "volume", 0) or 0) > 0
        return not illiquid and has_volume
    ...
    def _check_exit_conditions(self, ticker: str, prices: dict):
        ...
        entry_price = position.get("entry_price") or mid  # fallback to mid if unknown

        # Take-profit: maximum payout reached
        if bid >= 100:
            logger.info(f"TAKE-PROFIT (100¢ bid): {ticker} {side}")
            self._close_position(ticker, position, prices)
            return

        # Take-profit at 99¢ only if we have at least 1¢ profit
        ...
        # Emergency exit: illiquid and significantly underwater
    ...
    def _check_buying_opportunity(self, ticker: str, prices: dict):
        """Check if market meets all buying criteria. Only buy one side per ticker."""
        ...
        for side in ["yes", "no"]:
            if self._should_buy(ticker, side, prices, market):
                ...
                break  # never attempt both sides on the same ticker
    ...
    def _execute_buy(self, ticker: str, side: str, prices: dict, market):
        ...
        # Cooldown: don't retry within 10 seconds
        ...
        # Order price: bid for pure maker, bid+1 for faster fill
        ...
        entry_price_dollars = order_price / 100

        # Integer contract sizing — respect Kelly without forcing minimum 1
        desired_contracts     = round(position_capital / entry_price_dollars)
        max_affordable        = max(0, int(available_cash / entry_price_dollars))
        contracts             = min(desired_contracts, max_affordable)

        if contracts < 1:
            logger.debug(
                f"SKIP {ticker} {side}: Kelly says {desired_contracts} contracts, "
                f"can afford {max_affordable} (portfolio=${total_portfolio:.2f})"
            )
            return

        total_cost = contracts * entry_price_dollars
        ...
        try:
            ...
            # Record position only after confirmed order submission
            self.held_positions[ticker] = {
                "side": side,
                "contracts": contracts,
            ...
            self.buy_attempts.pop(ticker, None)  # clear so it can retry next tick
    ...
    def _close_position(self, ticker: str, position: dict, prices: Optional[dict] = None, emergency: bool = False):
        ...
        try:
            ...
            if emergency or spread >= 50:
                order_type = "market"
                exit_price = None
                logger.warning(f"Using MARKET ORDER for {ticker} (spread={spread}¢, emergency={emergency})")
            elif bid >= 1:
                order_type = "limit"
                exit_price = bid
            else:
            ...
            if entry_price and exit_price:
                ...
                price_str = "MARKET" if order_type == "market" else f"{exit_price}¢"
                logger.info(f"CLOSED {position['contracts']} {side} on {ticker} @ {price_str}")
